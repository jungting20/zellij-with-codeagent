...
import (
	...
	"zellij-with-codeagent/internal/chrome"
...
func Run(args []string, stdin io.Reader, stdout, stderr io.Writer, newClient ClientFactory, cfg Config) int {
	...
	if err != nil {
		fmt.Fprintf(stderr, "build chrome plan: %v\n", err)
		return 1
	}

	requestID := chrome.RequestID(payload.Session)
	...
	if err != nil {
		fmt.Fprintf(stderr, "chrome submit failed via socket %s: %v\n", *socketPath, err)
