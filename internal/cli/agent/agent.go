...
func printDashboardUsage(w io.Writer) {
	...
	fmt.Fprintln(w)
	fmt.Fprintln(w, "Keys: Tab/Shift+Tab switch areas, j/k move, 1-9 select across areas, n start agent from zoxide, a choose task alias, f follow-up queue, Space pin/unpin, d close unpinned agent pane, Enter focus and exit, R refresh, q quit")
}
