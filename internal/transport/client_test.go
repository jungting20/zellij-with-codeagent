...
func TestTaskAliasClientServerRoundTripAndValidation(t *testing.T) {
	service := newFakeRuntimeService()
	server := httptest.NewServer(newTestServer(t, service))
	defer server.Close()
	client := NewClient(ClientOptions{})
	client.baseURL, client.http = server.URL, server.Client()
	for _, alias := range []string{"review", "직접 입력 q", ""} {
	...
	_, err := client.SetAgentTaskAlias(context.Background(), "agent/1", SetAgentTaskAliasRequest{TaskAlias: "invalid\nlabel"})
	var clientErr *ClientError
	if !errors.As(err, &clientErr) || clientErr.StatusCode != http.StatusBadRequest {
		t.Fatalf("invalid alias should return HTTP 400: %v", err)
	}
}
