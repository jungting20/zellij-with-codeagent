...
manifest_path="$repo_root/plugins/agent-next-bridge/Cargo.toml"
artifact_path="$repo_root/plugins/agent-next-bridge/target/wasm32-wasip1/release/agent_next_bridge.wasm"
plugin_dir="${ZELLIJ_PLUGIN_DIR:-${XDG_CONFIG_HOME:-$HOME/.config}/zellij/plugins}"
rustup_toolchain="${AGENT_NEXT_RUSTUP_TOOLCHAIN:-1.88.0}"
temporary_file=""
...
if ! command -v rustup >/dev/null 2>&1; then
    echo "agent-next bridge requires Rustup toolchain $rustup_toolchain; PATH cargo is not used" >&2
    exit 1
fi
...
if [[ ! -s "$artifact_path" ]]; then
    echo "agent-next bridge artifact was not produced: $artifact_path" >&2
    exit 1
fi

mkdir -p -- "$plugin_dir"
plugin_dir="$(cd -- "$plugin_dir" && pwd -P)"
destination_path="$plugin_dir/agent-next-bridge.wasm"
temporary_file="$(mktemp "$plugin_dir/.agent-next-bridge.wasm.XXXXXX")"
