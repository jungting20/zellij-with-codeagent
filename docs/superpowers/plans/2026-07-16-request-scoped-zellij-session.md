...
- `internal/cli/ticketworker/ticketworker.go`, `internal/ticketworker/plan.go`, `internal/ticketworker/manager.go`: preserve the physical session through delayed worker creation.
...
### Task 7: Delayed Ticket-Worker and Chrome Pane Creation

**Files:**
- Modify: `internal/cli/ticketworker/ticketworker.go`
- Modify: `internal/cli/ticketworker/ticketworker_test.go`
- Modify: `internal/ticketworker/plan.go`
- Modify: `internal/ticketworker/plan_test.go`
...
- [ ] **Step 1: Add failing ticket-worker propagation tests**

Assert `ticket-worker start --zellij-session physical-a --dry-run` emits `zellij_session: physical-a`; the manager command contains `--zellij-session physical-a`; `ManagerOptions.ZellijSession` is `physical-a`; and launched workers contain:

```go
if got := creates[0].ZellijSession; got != "physical-a" {
	t.Fatalf("worker zellij session = %q, want physical-a", got)
}
```

- [ ] **Step 2: Add failing Chrome delayed-creation tests**
...
- [ ] **Step 3: Run delayed-creation tests and verify failures**

Run: `go test ./internal/cli/ticketworker ./internal/ticketworker ./cmd/agent-role/tabwatcher ./cmd/agent-role/tabnetwork -run 'Test.*ZellijSession' -count=1`

Expected: FAIL because delayed creators do not retain the physical session.

- [ ] **Step 4: Preserve ticket-worker session state**

Add `ZellijSession string` to `ticketworker.PlanRequest` and `ManagerOptions`. Validate both with `strings.TrimSpace`. Set `ExecutionPlanPayload.ZellijSession`. Append `--zellij-session <value>` to the manager command. Store it on `Manager` and include it in every worker `transport.CreatePaneRequest`.

In `ticket-worker start`, resolve the CLI flag using Task 1. In `ticket-worker manager`, require `--zellij-session` and pass it into `ManagerOptions`; this internal command receives the explicit value generated by the start plan.

- [ ] **Step 5: Preserve Chrome watcher session state**
...
- [ ] **Step 6: Format and run delayed-creation tests**

Run: `gofmt -w internal/cli/ticketworker internal/ticketworker cmd/agent-role/tabwatcher cmd/agent-role/tabnetwork && go test ./internal/cli/ticketworker ./internal/ticketworker ./cmd/agent-role/tabwatcher ./cmd/agent-role/tabnetwork -count=1`

Expected: PASS.

- [ ] **Step 7: Commit delayed-creation propagation**

```bash
git add internal/cli/ticketworker internal/ticketworker cmd/agent-role/tabwatcher cmd/agent-role/tabnetwork
...
Document these exact rules in the work, Chrome, planner, and ticket-worker sections:
