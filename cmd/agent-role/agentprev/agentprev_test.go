...
func TestPrepareAgentPrevCommand(t *testing.T) {
	binDir := t.TempDir()
	...
	want := []string{fakeBinary, "agent", "prev", "--socket", "/tmp/test.sock"}
	if !slices.Equal(cmd.Args, want) {
		t.Fatalf("cmd.Args = %#v, want %#v", cmd.Args, want)
	}
}

func TestRunAgentPrevReturnsChildExitCode(t *testing.T) {
	binDir := t.TempDir()
	fakeBinary := filepath.Join(binDir, "zellij-agent")
	if err := os.WriteFile(fakeBinary, []byte("#!/bin/sh\nexit 7\n"), 0o755); err != nil {
		t.Fatalf("write fake zellij-agent: %v", err)
